...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        ...
        await update.message.reply_text(
            ...
            "• /pdf atau /word — export jawapan terakhir aku jadi fail\n\n"
...
async def terms_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for i in range(0, len(TERMS_TEXT), 4000):
        await update.message.reply_text(TERMS_TEXT[i : i + 4000])


async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📖 Senarai Command StudyBot\n\n"
        "/start — mula/mulakan semula bot\n"
        "/credits — semak baki kredit kau\n"
        "/topup — beli kredit tambahan (ToyyibPay)\n"
        "/referral — dapatkan link jemput kawan, dapat kredit percuma\n"
        "/pdf — export jawapan terakhir aku jadi fail PDF\n"
        "/word — export jawapan terakhir aku jadi fail Word\n"
        "/reset — clear chat history, mula soalan baru\n"
        "/terms — terma & syarat, dasar privasi, dasar refund\n"
        "/support <mesej> — hubungi admin untuk bantuan/laporkan masalah\n\n"
        "Selain command, boleh terus taip soalan, atau hantar gambar/PDF/video untuk aku analyze! 📷📄🎥"
    )
...
async def support_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ Pesanan kau dah dihantar kepada admin. Kami akan reply secepat mungkin!"
    )
    ...
    forward_text = (
        f"📩 Support ticket baru\n\n"
        f"Dari: {user.first_name} (@{user.username or 'tiada_username'})\n"
        f"User ID: {user.id}\n\n"
        f"Mesej:\n{message_text}\n\n"
        f"— Reply guna: /reply {user.id} <jawapan kau>"
    )
