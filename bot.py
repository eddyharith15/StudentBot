...
- Admin dapat notifikasi automatik bila ada user baru /start, dan boleh semak
  ringkasan angka keseluruhan guna /stats
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        for admin_id in config.ADMIN_USER_IDS:
            try:
                username_display = f"@{user.username}" if user.username else "(tiada username)"
                referral_note = " (join via link referral)" if referral_bonus_applied else ""
                await context.bot.send_message(
                    chat_id=admin_id,
                    text=(
                        f"🆕 User baru join StudyBot!\n"
                        f"Nama: {user.first_name}\n"
                        f"Username: {username_display}\n"
                        f"ID: {user.id}{referral_note}"
                    ),
                )
            except Exception:
                pass  # admin mungkin belum start chat dengan bot
        ...
        await update.message.reply_text(
            ...
            "/reset untuk mula chat baru. Guna /help untuk senarai penuh command, "
            "/terms untuk terma & dasar privasi.\n\n"
            "Sebelum mula, sila baca dulu panduan ringkas macam mana nak guna StudyBot 👇",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("📖 Panduan Guna StudyBot", callback_data="help:manual")]]
            ),
        )
    else:
        await update.message.reply_text(
...
async def do_export(user, context: ContextTypes.DEFAULT_TYPE, fmt: str, args: list[str], reply_text_func, reply_document_func):
    ...
    if topic:
        ...
        try:
            ...
            answer = text_utils.clean_ai_text(response.text) if response.text else None
            if not answer:
...
MANUAL_TEXT = f"""📖 PANDUAN GUNA STUDYBOT

1️⃣ Apa StudyBot boleh buat?
• Jawab soalan pelajaran (semua subjek/tingkatan)
• Explain & pandu siapkan assignment/homework
• Ringkaskan nota panjang
• Baca & analisa gambar nota/soalan, fail PDF, dan video 📷📄🎥

2️⃣ Macam mana nak mula tanya?
• Taip terus soalan macam chat biasa
  Contoh: "terangkan proses fotosintesis"
• Nak tanya pasal gambar/PDF/video? Terus hantar fail tu
  (boleh tambah caption soalan sekali kalau nak)

3️⃣ Sistem kredit
• 1 kredit = 1 soalan/mesej/fail
• User baru dapat {config.FREE_STARTING_CREDITS} kredit PERCUMA bila /start
• /credits — semak baki kredit
• /topup — beli kredit tambahan bila dah habis

4️⃣ Export jawapan jadi fail (senang nak simpan/print/share)
• /pdf — jadikan fail PDF
• /word — jadikan fail Word (.docx)
• /ppt — jadikan slide PowerPoint
• /excel — jadikan spreadsheet Excel
Cara guna:
  a) Tanya soalan dulu → lepas AI jawab, taip /pdf (atau /word, /ppt, /excel)
  b) ATAU terus taip /ppt <topik>, contoh: "/ppt kitaran hujan" — StudyBot
     terus jana kandungan baru pasal topik tu & jadikan fail (guna 1 kredit)

5️⃣ Dapat kredit PERCUMA tanpa bayar
• /referral — dapat link khas untuk jemput kawan. Korang berdua dapat
  bonus kredit setiap kali ada kawan join guna link tu 🎁

6️⃣ Command berguna lain
• /reset — mula topik/chat baru (padam history perbualan sebelum ni)
• /terms — terma & dasar privasi
• /support <penerangan masalah> — hubungi admin terus
• /help — menu pantas dengan butang cepat

Ada masalah atau nak tanya apa-apa? Guna /support bila-bila masa, kitorang
akan bantu secepat mungkin 🙏

Selamat belajar dengan StudyBot! 🚀"""


async def send_manual(reply_func):
    for i in range(0, len(MANUAL_TEXT), 4000):
        await reply_func(MANUAL_TEXT[i : i + 4000])


async def manual_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await send_manual(update.message.reply_text)
...
def help_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("📖 Panduan Guna", callback_data="help:manual")],
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Command penuh: /start /manual /credits /topup /referral /pdf /word /ppt /excel /reset /terms /support",
...
Jawab dengan draft je, tanpa pembukaan/penutup tambahan."""
    ...
    try:
        ...
        draft = (
            text_utils.clean_ai_text(response.text)
            if response.text
            else "Tak dapat jana draft, cuba lagi."
        )
    except Exception as e:
...
async def stats_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin je boleh guna: /stats — ringkasan jumlah user, kredit, revenue, dll."""
    caller_id = update.effective_user.id
    if caller_id not in config.ADMIN_USER_IDS:
        await update.message.reply_text("Command ni untuk admin je 🙅")
        return
    ...
    await update.message.reply_text(
        "📊 STATS STUDYBOT\n\n"
        f"👥 Jumlah user: {s['total_users']}\n"
        f"🆕 User baru hari ni: {s['new_today']}\n"
        f"🆕 User baru 7 hari lepas: {s['new_7d']}\n\n"
        f"💳 Jumlah kredit user (semua): {s['total_credits']}\n\n"
        f"💰 Transaksi berjaya: {s['paid_tx_count']} (RM{s['paid_tx_total_myr']:.2f})\n\n"
        f"🎁 Jumlah referral berjaya: {s['total_referrals']}\n"
        f"🎫 Tiket support terbuka: {s['open_tickets']}"
    )
...
async def process_media(
    ...
    try:
        ...
        reply_text = (
            text_utils.clean_ai_text(response.text)
            if response.text
            else "Maaf, tak dapat proses fail ni. Cuba hantar semula."
        )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reply_text = (
            text_utils.clean_ai_text(response.text)
            if response.text
            else "Maaf, tak dapat jana jawapan. Cuba tanya lain cara."
        )
    except Exception as e:
...
async def main():
    ...
    application.add_handler(CommandHandler("manual", manual_cmd))
