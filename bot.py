...
# --- OpenRouter (backup terakhir, guna model free — Qwen/Llama) ---
# Optional: kalau OPENROUTER_API_KEY tak di-set, bot skip step ni sepenuhnya (tak error).
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY")
openrouter_client = (
    OpenAIClient(base_url="https://openrouter.ai/api/v1", api_key=OPENROUTER_API_KEY)
    if OPENROUTER_API_KEY
    else None
)

# Model free OpenRouter berubah dari semasa ke semasa (provider tukar/rotate tanpa notis),
# so kita senaraikan beberapa + "openrouter/free" (auto-router OpenRouter sendiri) sebagai
# jaring keselamatan paling akhir.
...
def contents_have_media(contents: list[dict]) -> bool:
    """Semak kalau ada gambar/fail (inline_data) dalam contents — OpenRouter free model
    text-only kita tak boleh handle media, so skip fallback OpenRouter untuk kes ni."""
    for msg in contents:
...
def gemini_contents_to_openai_messages(contents: list[dict]) -> list[dict]:
    """Tukar format contents Gemini (role/parts) ke format messages OpenAI-compatible
    (role/content) yang OpenRouter perlukan."""
...
def generate_with_fallback(contents: list[dict]):
    """Cuba setiap model Gemini dulu. Kalau SEMUA gagal (contoh quota habis serentak),
    cuba backup OpenRouter (Qwen/Llama, free) — tapi hanya untuk teks je, bukan gambar/fail."""
    last_error = None
    ...
    # Semua Gemini gagal — cuba OpenRouter kalau ada key & bukan request media
    if openrouter_client and not contents_have_media(contents):
        openai_messages = gemini_contents_to_openai_messages(contents)
        for model_name in OPENROUTER_MODEL_CANDIDATES:
            try:
                ...
                logger.info(f"Fallback OpenRouter ({model_name}) berjaya.")
                return SimpleNamespace(text=answer_text)  # samakan interface dgn response Gemini
            except Exception as e:
                logger.warning(f"OpenRouter model {model_name} gagal ({e}), cuba seterusnya...")
                last_error = e
...
# Track "menu action" yang user sedang tunggu input untuk (contoh: user tekan
# "🎨 Generate Gambar", kita simpan state "image", so mesej SETERUSNYA dari dia
# akan dianggap sebagai penerangan gambar, bukan soalan chat biasa.
user_pending_action: dict[int, str] = {}

MENU_IMAGE = "🎨 Generate Gambar"
MENU_PDF = "📄 Generate PDF"
MENU_DOC = "📝 Generate Word"
MENU_CREDITS = "💳 Kredit Saya"
MENU_TOPUP = "🔝 Topup Kredit"
MENU_RESET = "🔄 Reset Chat"
...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    """Menu butang tetap kat bawah chat — senang tekan, tak payah ingat command."""
    buttons = [
        [MENU_IMAGE, MENU_PDF],
        [MENU_DOC, MENU_CREDITS],
        [MENU_TOPUP, MENU_RESET],
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        await update.message.reply_text(
            ...
            "• Jawab soalan pelajaran & pandu assignment\n"
            "• Ringkaskan nota panjang\n"
            "• Baca gambar/PDF/Word/PowerPoint/Excel/audio/video yang kau hantar 📷📄🎥🎧\n"
            "• Generate gambar AI, PDF, atau Word — guna butang menu bawah ni 👇\n\n"
            "1 kredit = 1 soalan/mesej. Taip terus soalan kau, atau tekan mana-mana butang!",
            reply_markup=main_menu_keyboard(),
        )
    else:
        await update.message.reply_text(
            f"Hai balik {user.first_name}! Baki kredit kau: {credits} 💳\n"
            "Taip soalan kau, atau guna butang menu bawah ni.",
            reply_markup=main_menu_keyboard(),
        )
...
# ---------- Generate Content (gambar / PDF / Word) — fungsi teras (dipakai command & menu) ----------

async def run_image_generation(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt_text: str):
...
async def generate_answer_text(prompt_text: str) -> str:
    """Panggil Gemini untuk dapatkan jawapan teks (dipakai oleh PDF dan Word generation)."""
...
async def run_pdf_generation(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt_text: str):
...
async def run_doc_generation(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt_text: str):
...
# ---------- Command versi (/image, /pdf, /doc) — tetap ada untuk power user ----------
...
async def image_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt_text:
        await update.message.reply_text(
            ...
            "Atau guna butang menu 🎨 Generate Gambar kat bawah!"
        )
        return
    await run_image_generation(update, context, prompt_text)
...
async def pdf_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt_text:
        await update.message.reply_text(
            ...
            "Atau guna butang menu 📄 Generate PDF kat bawah!"
        )
        return
    await run_pdf_generation(update, context, prompt_text)
...
async def doc_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt_text:
        await update.message.reply_text(
            ...
            "Atau guna butang menu 📝 Generate Word kat bawah!"
        )
        return
    await run_doc_generation(update, context, prompt_text)
...
# Gemini tak boleh baca fail Office (zip-based format) terus macam PDF, so kita extract
# teks dulu guna library Python, baru hantar teks tu ke Gemini sebagai prompt biasa.
...
# mime_type -> (extractor function, label untuk mesej)
OFFICE_EXTRACTORS = {
    "application/vnd.openxmlformats-officedocument.wordprocessingml.document": (extract_docx_text, "Word"),
    "application/vnd.openxmlformats-officedocument.presentationml.presentation": (extract_pptx_text, "PowerPoint"),
    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": (extract_xlsx_text, "Excel"),
}

MAX_EXTRACTED_CHARS = 30000  # elak prompt terlalu panjang (had context & kos)


async def process_office_document(update: Update, context: ContextTypes.DEFAULT_TYPE, file_id: str, extractor, label: str):
    """Download fail Office, extract teks, hantar teks tu ke Gemini macam soalan biasa."""
    user = update.effective_user
    user_id = user.id
    ...
    try:
        ...
        if tg_file.file_size and tg_file.file_size > MAX_FILE_SIZE_MB * 1024 * 1024:
            ...
            await update.message.reply_text(
                f"⚠️ Fail ni terlalu besar (max {MAX_FILE_SIZE_MB}MB). Cuba compress dulu ye."
            )
            return
        ...
        if not extracted_text.strip():
            await database.add_credits(user_id, 1)
            await update.message.reply_text(
                f"⚠️ Tak jumpa teks dalam fail {label} ni (mungkin fail kosong atau imej je). "
                "Cuba fail lain ye."
            )
            return

        if len(extracted_text) > MAX_EXTRACTED_CHARS:
            extracted_text = extracted_text[:MAX_EXTRACTED_CHARS] + "\n\n[...dipotong, fail terlalu panjang...]"

        user_caption = (update.message.caption or "").strip()
        instruction = user_caption or (
            f"Ni kandungan fail {label} yang student hantar. Ringkaskan isi penting dalam "
            "bentuk bullet points yang senang diingati untuk exam."
        )
        prompt_text = f"{instruction}\n\n--- Kandungan fail {label} ---\n{extracted_text}"

        answer_text = await generate_answer_text(prompt_text)

    except Exception as e:
        logger.error(f"Office document processing error ({label}): {e}")
        await database.add_credits(user_id, 1)
        await update.message.reply_text(
            f"Alamak, gagal proses fail {label} ni. Cuba lagi sekejap ye 🙏"
        )
        return
    ...
    for i in range(0, len(answer_text), 4000):
        chunk = answer_text[i : i + 4000]
        is_last = i + 4000 >= len(answer_text)
        await update.message.reply_text(chunk + (footer if is_last else ""))


# ---------- Media Handlers (gambar / PDF / video / audio / Office docs) ----------
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mime_type in OFFICE_EXTRACTORS:
        extractor, label = OFFICE_EXTRACTORS[mime_type]
        await process_office_document(update, context, doc.file_id, extractor, label)
        return

    await update.message.reply_text(
        "⚠️ Buat masa ni bot ni sokong fail **PDF, Word (.docx), PowerPoint (.pptx), "
        "dan Excel (.xlsx)** je. Untuk jenis fail lain, cuba export/save ke salah satu "
        "format tu dulu ye."
    )
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Voice note Telegram (rakaman push-to-talk dalam app)."""
    voice = update.message.voice
    mime_type = voice.mime_type or "audio/ogg"
    await process_media(
        update, context, voice.file_id, mime_type,
        default_prompt=(
            "Ni voice note/soalan bersuara dari student. Dengar dan jawab soalan dia, atau "
            "kalau ni nota bersuara, boleh ringkaskan isi penting."
        ),
    )


async def handle_audio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Fail audio (contoh rakaman kuliah/lecture yang di-upload, bukan voice note)."""
    audio = update.message.audio
    mime_type = audio.mime_type or "audio/mpeg"
    await update.message.reply_text("🎧 Audio diterima, sedang proses...")
    await process_media(
        update, context, audio.file_id, mime_type,
        default_prompt=(
            "Ni fail audio (contoh rakaman kuliah/lecture) yang student hantar. Ringkaskan "
            "isi penting dalam bentuk bullet points, kecuali student minta benda lain "
            "dalam caption."
        ),
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 1. Handle tekan butang menu ---
    if user_text == MENU_IMAGE:
        user_pending_action[user_id] = "image"
        await update.message.reply_text(
            "🎨 Ok! Taip penerangan gambar yang kau nak jana:\n"
            "(Contoh: gambarajah kitaran air, kucing comel warna oren)"
        )
        return
    if user_text == MENU_PDF:
        user_pending_action[user_id] = "pdf"
        await update.message.reply_text(
            "📄 Ok! Taip soalan atau topik untuk PDF tu:\n(Contoh: ringkaskan bab fotosintesis)"
        )
        return
    if user_text == MENU_DOC:
        user_pending_action[user_id] = "doc"
        await update.message.reply_text(
            "📝 Ok! Taip soalan atau topik untuk Word doc tu:\n"
            "(Contoh: buatkan nota sejarah kemerdekaan Malaysia)"
        )
        return
    if user_text == MENU_CREDITS:
        await credits_cmd(update, context)
        return
    if user_text == MENU_TOPUP:
        await topup_cmd(update, context)
        return
    if user_text == MENU_RESET:
        await reset(update, context)
        return

    # --- 2. Kalau user ada "pending action" (baru tekan butang generate) ---
    pending = user_pending_action.pop(user_id, None)
    if pending == "image":
        await run_image_generation(update, context, user_text)
        return
    if pending == "pdf":
        await run_pdf_generation(update, context, user_text)
        return
    if pending == "doc":
        await run_doc_generation(update, context, user_text)
        return

    # --- 3. Chat biasa (soalan/assignment) ---
    await database.ensure_user(user_id, user.username or user.first_name, config.FREE_STARTING_CREDITS)
