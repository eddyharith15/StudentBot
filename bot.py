...
- /pdf, /word, dan /ppt — export jawapan terakhir AI jadi fail PDF/Word/PowerPoint
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new:
        ...
        await update.message.reply_text(
            ...
            "• /pdf, /word atau /ppt — export jawapan terakhir aku jadi fail\n\n"
...
async def export_pdf_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not answer:
        await update.message.reply_text(
            "Tak ada jawapan lagi untuk export. Tanya soalan dulu, lepas tu guna /pdf 😊"
        )
        return

    await update.message.reply_text("Sedang jana fail PDF...")
    try:
        pdf_bytes = document_export.text_to_pdf_bytes("StudyBot - Nota", answer)
...
async def export_word_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not answer:
        await update.message.reply_text(
            "Tak ada jawapan lagi untuk export. Tanya soalan dulu, lepas tu guna /word 😊"
        )
        return

    await update.message.reply_text("Sedang jana fail Word...")
    try:
        docx_bytes = document_export.text_to_docx_bytes("StudyBot - Nota", answer)
        await update.message.reply_document(
        ...
        logger.error(f"Gagal generate Word: {e}")
        await update.message.reply_text("Alamak, gagal jana fail Word. Cuba lagi sekejap ye 🙏")


async def export_pptx_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    answer = get_last_ai_answer(user_id)
    if not answer:
        await update.message.reply_text(
            "Tak ada jawapan lagi untuk export. Tanya soalan dulu, lepas tu guna /ppt 😊"
        )
        return

    await update.message.reply_text("Sedang jana fail PowerPoint...")
    try:
        pptx_bytes = document_export.text_to_pptx_bytes("StudyBot - Nota", answer)
        await update.message.reply_document(
            document=io.BytesIO(pptx_bytes),
            filename="studybot_nota.pptx",
            caption="📊 Ni slide PowerPoint dari jawapan terakhir aku!",
        )
    except Exception as e:
        logger.error(f"Gagal generate PowerPoint: {e}")
        await update.message.reply_text("Alamak, gagal jana fail PowerPoint. Cuba lagi sekejap ye 🙏")
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Command penuh: /start /credits /topup /referral /pdf /word /ppt /reset /terms /support",
...
async def main():
    ...
    application.add_handler(CommandHandler("ppt", export_pptx_cmd))
